...
url=urllib.urlopen('http://store.steampowered.com/stats/').read()
steampage = BeautifulSoup(url,'lxml')

timestamp = time.time()
currentTime = datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')

top100CSV = open('SteamTop100byTime.csv', 'w')
top100CSV.write('time,id,name,current,max\n')
...
steamOverallCSV = open('SteamOverallbyTime.csv', 'w')
